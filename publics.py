...
def check_and_raise(module):
    import subprocess
    result = []
    for item in subprocess.getstatusoutput('systemctl status %s' % module)[1].split('\n'):
        result.append(item.split())
    if result[2][1] != 'active':
        try:
            subprocess.run(['systemctl', 'restart', module])
        except:
            pass
...
def db():
    ...
    # con = MongoClient('mongodb://mongodb')
    return con[db_name]
...
def es():
    ...
    # return Elasticsearch('http://' + elastic)
    # return Elasticsearch('htps://5.63.9.73')
    # return Elasticsearch('http://elasticsearch')
    return Elasticsearch('localhost')


def es45():
    from elasticsearch import Elasticsearch
    return Elasticsearch('http://45.93.170.150')
    # return Elasticsearch('localhost')
